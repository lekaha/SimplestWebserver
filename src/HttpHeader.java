...
public class HttpHeader extends HttpPacket {
    private Hashtable mHeaderFields;

    public HttpHeader() {}

    public HttpHeader(Hashtable hashtable) {
        mHeaderFields = hashtable;
    }

    public static HttpHeader parse(HttpRequest request) {
        final HttpParser parser = new HttpParser(request.getInputStream());
        return new HttpHeader(parser.getHeaders());
    }

    public static HttpHeader generate(HttpResponse response) {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        try {
            outputStream.write(response.getOutputByteBuffer().array());
            final HttpParser parser = new HttpParser(new ByteArrayInputStream(outputStream.toByteArray()));
            String rep = parser.getHttpReply(response.getResponseCode().getCode());
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        public int parseRequest() throws IOException {
            ...
            if (cmd[0].equals("GET") || cmd[0].equals("HEAD")) {
                ...
                if (idx < 0) url = cmd[1];
                    ...
                    url = URLDecoder.decode(cmd[1].substring(0, idx), "UTF-8");
